"""
This code was taken from https://github.com/run-llama/llama_index/blob/b224ff42d045dd2c3839c03a5c9e35f4c1303e79/llama-index-integrations/readers/llama-index-readers-web/llama_index/readers/web/beautiful_soup_web/base.py#L134

Changes are marked with # Modification:
...
class CachingBeautifulSoupWebReader(BasePydanticReader):
    ...
    def load_data(
        ...
        session: Optional[Session] = None,  # Modification: This parameter was added.
    ) -> List[Document]:
        ...
        for url in urls:
            try:
                # Modification: Used a Session if one was passed in.  Also, provides a timeout on the request.
                requestor = session if session else requests
                page = requestor.get(url, timeout=10)
            except Exception as e:
                raise ValueError(f"Error loading url: {url}.", e)
