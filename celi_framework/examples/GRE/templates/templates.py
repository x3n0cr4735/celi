...
def score_llm_output(prompt:str, llm_output: str, rubric: dict) -> int:
    f"""
    Evaluates the output of a large language model (LLM) based on a provided rubric.

    Args:
    llm_output (str): The text output generated by the LLM.
    rubric (dict): A dictionary containing the scoring criteria and the corresponding expectations.
                   Each key is a criterion, and the value is another dictionary with 'description' and 'score_range'.

    Returns:
    int: The total score for the LLM output based on the rubric.
